...
for val_test in val_tests:
    ...
    ESR = calculate_esr(tan_delta, fsw, C*1e-6)
...
def run_simulations(simStructs):
    ...
    for i, opts in enumerate(simStructs):
        try:
            print(f"Running simulation {i + 1}/{len(simStructs)}...")
            ...
            average_current, delta_current, delta_current_percentage = calculate_current_1(current)
            average_voltage, delta_voltage, delta_voltage_percentage = calculate_voltage_1(voltage)
